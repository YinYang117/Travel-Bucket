...
function EditTripForm({ hideModal, trip }) {
    ...
    let placeholder2 = newDate.toISOString().slice(0, 10);
    let placeholder = newDate.getFullYear() + "-" + newDate.getMonth() + "-" + newDate.getDay()
    //   let document.querySelector("#today").value = today;
    ...
    return (
        <div className="formContainer3">
            <form
                ...
                <input onChange={e => setImageUrl(e.target.value)} id="date" name="date" type="text" className="new-trip-image" placeholder={trip?.imageUrl} value={imageUrl} />
                ...
                <input onChange={e => setStartDate(e.target.value)} type="date" className="new-trip-start-date" placeholder={placeholder2} value={startDate} />
                <label className='label'>
                    Trip End:
                </label>
